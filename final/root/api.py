...
def refresh_access_token(refresh_token):
    ...
    if response.status_code == 200:
        ...
        print("فشل تحديث التوكن، يجب تسجيل الدخول مجددًا.")
        return None
...
def add_to_wishlist(qr_number):
    ...
    if response.status_code in (200, 201):
        print(f"✅ المنتج برقم QR {qr_number} تم إضافته إلى الوِشلست.")
        return True
    else:
        print(f"❌ فشل في الإضافة:\n{response.status_code} - {response.text}")
        return False
...
def remove_from_wishlist(QrNumber):
    ...
    if response.status_code == 200 or response.status_code == 204:
        print(f"✅ المنتج برقم QR {QrNumber} تم إزالته من الوِشلست.")
        return True
    else:
        print(f"❌ فشل في الحذف:\n{response.status_code} - {response.text}")
        return False
...
def send_checkout_request():
    ...
    try:
        response = requests.post(url, json=payload)
        response.raise_for_status()  # يتأكد أن الرد ناجح (كود 200-299)
        return response.json()
...
def get_products_by_category_by_name(category_name):
    ...
    response = requests.get(url)
    response.raise_for_status()  # يطبع الخطأ في حال فشل الطلب
    return response.json()
def get_category_details(category_name):
    url = f"{BASE_URL}/product/categories/{category_name}/details/"
    try:
        response = requests.get(url)
        response.raise_for_status()  # هذا السطر يرفع استثناء لو الطلب فشل
        return response.json()       # ترجع البيانات بصيغة JSON (عادة dict)
...
def fetch_wishlist():
    try:
        url = f"{BASE_URL}/product/wishlist/getmywish/"  # استبدليه بالرابط الحقيقي
        response = requests.get(url)
        if response.status_code == 200:
...
def extract_cart_data(json_data):
    ...
    # EasyCartVirtualItems (اللي لسه في السلة)
    ...
    # EasyCartItems (اللي اتحطوا في الكارت)
...
def get_cart_items():
    ...
    if not products:
        print("No virtual products found.")
        return
    # print(products, picked_items)
